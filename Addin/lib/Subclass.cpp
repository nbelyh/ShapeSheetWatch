...
LRESULT CALLBACK HookWndProc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp)
{
	...
	ASSERT(pSubclassWnd); if (!pSubclassWnd) return ::DefWindowProc(hwnd, msg, wp, lp);
...
void CSubclassWndMap::Add(HWND hwnd, CSubclassWnd* pSubclassWnd)
{
	...
	if (pSubclassWnd->m_pNext==NULL) {
		...
		pSubclassWnd->m_pOldWndProc = 
			(WNDPROC)SetWindowLongPtr(hwnd, GWLP_WNDPROC, (DWORD_PTR)HookWndProc);
...
void CSubclassWndMap::Remove(CSubclassWnd* pUnHook)
{
	...
	if (pHook==pUnHook) {
		...
		if (pHook->m_pNext)
			...
			RemoveKey(hwnd);
			SetWindowLongPtr(hwnd, GWLP_WNDPROC, (DWORD_PTR)pHook->m_pOldWndProc);
		}
	} else {
...
CSubclassWnd* CSubclassWndMap::Lookup(HWND hwnd)
{
	...
	if (!CMapPtrToPtr::Lookup(hwnd, (void*&)pFound))
		return NULL;
	ASSERT(dynamic_cast<CSubclassWnd*>(pFound) != NULL);
	return pFound;
}
