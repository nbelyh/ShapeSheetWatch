...
void CGridCtrl::OnSysColorChange()
{
	...
	if (GetGridBkColor() == m_cr3DFace)
		SetGridBkColor(::GetSysColor(COLOR_3DFACE));
...
void CGridCtrl::MergeAwareDrawCell (CGridCellBase* pCell, const CRect& rect, int row, int col, CDC* pDC)
{
	...
	{
		pDC->MoveTo(border.right+1, border.top);
		pDC->LineTo(border.right+1, border.bottom+1);
	}
	if (m_nGridLines == GVL_BOTH || m_nGridLines == GVL_HORZ)
	{
		pDC->MoveTo(border.left, border.bottom+1);
		pDC->LineTo(border.right+1, border.bottom+1);
	}
}

void CGridCtrl::MergeAwareDraw (const CRect& rect, int row, int col, CDC* pDC)
{
...
void CGridCtrl::OnDraw(CDC* pDC)
{
	...
	for (row = minVisibleRow; row <= maxVisibleRow; row++)
	{
		...
		for (col = minVisibleCol; col <= maxVisibleCol; col++)
		{
			...
			MergeAwareDraw(rect, row, col, pDC);
		}
	}
...
CCellID CGridCtrl::SetFocusCell(CCellID cell)
{
	...
	pCell = GetCell(cell);
	if(pCell != NULL && pCell->IsMerged())
		cell = pCell->GetMergeRange().GetTopLeft();
								...
								BOOL bSelectPastedCells /*=TRUE*/)
{
	...
	do
	{
		...
		while (!strLine.IsEmpty())
		{
			...
			{
				strCellText.Trim(CHARSET_SEPARATORS);
...
BOOL CGridCtrl::SetItem(const GV_ITEM* pItem)
{
	...
	if (pItem->mask & GVIF_MERGE_RANGE)
...
BOOL CGridCtrl::GetItem(GV_ITEM* pItem)
{
	...
	if ( pItem->mask & GVIF_MERGE_RANGE)
		pItem->mergeRange = pCell->GetMergeRange();
...
void CGridCtrl::FitVisible(bool resize_fixed /*= false*/)
{
	CDC* pDC = GetDC();
	if (!pDC)
		return;
	...
	if(!VisibleCells.IsValid())
		return;

	int nCol, nRow;

	int nStartColumn = VisibleCells.GetMinCol();
	int nEndColumn = VisibleCells.GetMaxCol();
	int nStartRow = VisibleCells.GetMinRow();
	int nEndRow   = VisibleCells.GetMaxRow();

	if (GetVirtualMode())
		SendCacheHintToParent(CCellRange(nStartRow, 0, nEndRow, nEndColumn - nStartColumn + 1));
	...
	CSize size;
	int fixed_min_lenght = 0;

	for (nCol = 0; nCol < GetFixedColumnCount(); nCol++)
	{
		...
		if( GetColumnWidth( nCol) > 0 )
		{
			...
			for (nRow = 0; nRow < GetRowCount(); nRow++)
			{
				if( GetRowHeight( nRow) > 0 )
				{
					...
					{
						if(size.cx > fixed_min_lenght)
							fixed_min_lenght = min(size.cx, (int)m_arColWidths[nCol]);
					}
					if (size.cy >(int) m_arRowHeights[nRow])
						m_arRowHeights[nRow] = size.cy;
				}
			}
			if(fixed_min_lenght > 0)
			{
				m_arColWidths[nCol] = fixed_min_lenght;
				fixed_min_lenght = 0;
			}
		}
	}

	for (nCol = GetFixedColumnCount(); nCol < GetColumnCount(); nCol++)
	{
		//  Don't size hidden columns or rows
		if( GetColumnWidth( nCol) > 0 )
		{
			...
			if(resize_fixed)
				m_arColWidths[nCol] = 0;
			for (nRow = 0; nRow < GetFixedRowCount(); nRow++)
			{
				if( GetRowHeight( nRow) > 0 )
				{
					CGridCellBase* pCell = GetCell(nRow, nCol);
	...
	for (nCol = nStartColumn; nCol <= nEndColumn; nCol++)
	{
		//  Don't size hidden columns or rows
		if( GetColumnWidth( nCol) > 0 )
		{
			...
			if (m_bTitleTips)
			{
				...
				if (pCell)
				{
					...
					if (!m_bRMouseButtonDown
						...
						&& (idCurrentCell.row < GetFixedRowCount() || idCurrentCell.col < GetFixedColumnCount())
						)
					{
		...
		case MOUSE_SELECT_CELLS:
			{
				...
				if (idCurrentCell != GetFocusCell() && idCurrentCell.row >= m_nFixedRows && idCurrentCell.col >= m_nFixedCols)
				{
...
void CGridCtrl::OnFixedRowClick(CCellID& cell)
{
	...
	if (GetFixedColumnSelection())
	{
		...
		else 
		{
			...
			CGridCellBase* base_cell = GetCell(cell);
			if (base_cell->IsMerged())
				cell.col = base_cell->GetMergeRange().GetMaxCol();
...
void CGridCtrl::OnFixedColumnClick(CCellID& cell)
{
	...
	if (GetFixedRowSelection())
	{
		...
		else
		{
			...
			CGridCellBase* base_cell = GetCell(cell);
			if (base_cell->IsMerged())
				cell.row = base_cell->GetMergeRange().GetMaxRow();
...
CCellID CGridCtrl::GetMergeCellID(CCellID cell)
{
	...
	if(pCell && pCell->IsMerged())
		return pCell->GetMergeRange().GetTopLeft();
	return cell;
}
//Merge the selected cells 
//by Huang Wei
...
bool CGridCtrl::GetCellValue(CCellID cell, CGridCellBase& to)
{
	return GetCellValue(cell.row, cell.col, to);
}
//Merge the selected cells 
//by Huang Wei
