...
public:
    ...
    BOOL Create(const RECT& rect, CWnd* parent, UINT nID, DWORD dwStyle);
...
public:
    ...
    bool GetCellValue(int nRow, int nCol, CGridCellBase& cell) const; //Get values of a cell
...
public:
	...
	void FitVisible(bool resize_fixed = false);
...
public:
	...
	bool GetCellValue(CCellID cell, CGridCellBase& to);
...
protected:
	...
	void MergeAwareDraw( const CRect& rect, int row, int col, CDC* pDC );
	void MergeAwareDrawCell ( CGridCellBase* pCell, const CRect &rect, int row, int col, CDC* pDC );
...
inline CGridCellBase* CGridCtrl::GetCell(int nRow, int nCol) const
{
    ...
    if (GetVirtualMode())
    {
		...
		cell.SetMergeRange(gvdi.item.mergeRange);
...
inline bool CGridCtrl::GetCellValue(int nRow, int nCol, CGridCellBase& cell) const
{
	if (nRow < 0 || nRow >= m_nRows || nCol < 0 || nCol >= m_nCols) 
		return false;
	...
	CGridCellBase* original = pRow->GetAt(m_arColOrder[nCol]);

	if (original == NULL)
		return false;

	cell.SetState(original->GetState());
	cell.SetFormat(original->GetFormat());
	cell.SetImage(original->GetImage());
	cell.SetBackClr(original->GetBackClr());
	cell.SetTextClr(original->GetTextClr());
	cell.SetData(original->GetData());
	cell.SetFont(original->GetFont());
	cell.SetMargin(original->GetMargin());
	cell.SetText(original->GetText());
	cell.SetGrid((CGridCtrl*)this);
	cell.SetMergeRange(original->GetMergeRange());
