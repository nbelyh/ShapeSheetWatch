...
BOOL CGridCellBase::Draw(CDC* pDC, int nRow, int nCol, CRect rect,  BOOL bEraseBkgnd /*=TRUE*/)
{
    ...
    if (IsFixed() && pGrid->GetGridLines() != GVL_NONE)
    {
        ...
        if (bHiliteFixed)
        {
			...
			CBrush darkbrush(RGB(182,182,182));
			pDC->FillRect(rect, &darkbrush);
			pDC->SetTextColor(::GetSysColor(COLOR_HIGHLIGHTTEXT));
        ...
        // {
            CPen lightpen(PS_SOLID, 1,  ::GetSysColor(COLOR_3DHIGHLIGHT)),
                darkpen(PS_SOLID,  1, RGB(192,192,192)),
        ...
        if (pGrid->GetImageList()->GetImageInfo(GetImage(), &Info))
        {
            ...
            */
            ...
            if ((nImageWidth + rect.left <= rect.right) && (nImageHeight + rect.top <= rect.bottom))
			{
				...
				{
					pGrid->GetImageList()->DrawIndirect(
						pDC, GetImage(), rect.CenterPoint() - CSize(nImageWidth / 2, nImageHeight / 2),
						CSize(nImageWidth,nImageHeight), CPoint(0, 0),
						ILD_NORMAL, SRCCOPY, CLR_NONE, pDC->GetBkColor());
				}
				else
				{
					pGrid->GetImageList()->DrawIndirect(
						pDC, GetImage(), rect.TopLeft(), CSize(nImageWidth,nImageHeight), CPoint(0, 0),
						ILD_NORMAL, SRCCOPY, CLR_NONE, pDC->GetBkColor());
				}
			}
            //   pGrid->GetImageList()->Draw(pDC, GetImage(), rect.TopLeft(), ILD_NORMAL);
...
BOOL CGridCellBase::GetTipTextRect( LPRECT pRect)
{ 
	CSize size = GetTextExtent(GetText());
	pRect->bottom = pRect->top + size.cy + GetMargin();
